...
#' 
#' See
#' \url{https://gdal.org/en/release-3.11/programs/gdal_vsi_sozip_create.html}
#' for detailed GDAL documentation.
#' @param input Input filenames (Character vector) (required). Can also be a [gdal_job]
#' object to extend a pipeline
#' @param output Output ZIP filename (required)
#' @param overwrite Whether overwriting existing output is allowed (Logical) (Default:
#' `false`)
#' @param recursive Travels the directory structure of the specified directories
#' recursively (Logical)
#' @param no_paths Store just the name of a saved file, and do not store directory
#' names (Logical)
#' @param enable_sozip Whether to automatically/systematically/never apply the SOZIP
#' optimization. Choices: "auto", "yes", "no" (Default: `auto`)
#' @param sozip_chunk_size Chunk size for a seek-optimized file. Format: `value in
#' bytes or with K/M suffix` (Default: `32768`)
#' @param sozip_min_file_size Minimum file size to decide if a file should be
#' seek-optimized. Format: `value in bytes or with K/M/G suffix` (Default: `1
#' MB`)
...
#' https://gdal.org/programs/gdal-vsi-sozip-create.html
